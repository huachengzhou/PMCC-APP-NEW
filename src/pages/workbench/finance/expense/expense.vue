		...
		<uni-forms :modelValue="formData" ref="form" validate-trigger="bind" err-show-type="undertext" :rules="rules">
			<uni-group title="基本信息" mode="card">
				<uni-forms-item name="projectId" label="所属项目" label-align="right">
					<project-picker ref="project" :multiple="false" :inputValue="projectValue" @getResults="getProjects"
						@getdrawType="getProjectDrawType"></project-picker>
				</uni-forms-item>
				<uni-forms-item required name="costMain" label="单位" label-align="right">
					<picker @change="changeCostMain" :value="costMainIndex" :range="costMainList" range-key="dicName">
						<view v-if="costMainList.length > 0" class="uni-input picker-border">
							{{costMainList[costMainIndex].dicName}}
						</view>
					</picker>
				</uni-forms-item>
				<uni-forms-item required name="costBillType" label="发票类型" label-align="right">
					<picker @change="changeCostBillType" :value="costBillTypeIndex" :range="costBillTypeList"
						range-key="name">
						<view v-if="costBillTypeList.length > 0" class="uni-input picker-border">
							{{costBillTypeList[costBillTypeIndex].name}}
						</view>
					</picker>
				</uni-forms-item>
				...
				<uni-forms-item required name="expenseDate" label="开票日期" label-align="right">
					<uni-datetime-picker type="date" v-model="formData.expenseDate" @change="dateChange">
					</uni-datetime-picker>
				</uni-forms-item>
				<uni-forms-item  required name="amount" label="金额(元)" label-align="right">
					<uni-easyinput @input="inputCalculate" type="digit" v-model="formData.amount"
						placeholder="金额"></uni-easyinput>
				</uni-forms-item>
				<uni-forms-item required name="taxes" label="税费(元)" label-align="right">
					<uni-easyinput @input="inputCalculate" type="digit" v-model="formData.taxes" placeholder="税费">
					</uni-easyinput>
				</uni-forms-item>
				<uni-forms-item required name="costTaxesTotal" label="价税合计(元)" label-align="right">
					<uni-easyinput @input="inputValue($event);" type="digit" v-model="formData.costTaxesTotal"
						placeholder="价税合计"></uni-easyinput>
				</uni-forms-item>
				...
				<uni-forms-item required name="file" label="附件" label-align="right">
					<common-file-picker  :orcType="'invoice'" :appKey="appkey"
						:dataObj="setFileFieldsName()"></common-file-picker>
				</uni-forms-item>
			</uni-group>
		</uni-forms>
...
<script>
	...
	console.log(utils.formatTimeNewBillZh("2017年11月13")) ;
	console.log(utils.formatTimeNewBillZh("2017年1月3")) ;
	console.log(utils.formatTimeNewBillZh("2017年1月3日")) ;
	console.log(utils.formatTimeNewBillZh("2017年12月30日")) ;
	console.log(utils.formatTimeNewBillZh("2017-11-13")) ;
	console.log(utils.formatTimeNewBillZh("2017-6-1")) ;
	...
	export default {
				...
				uni.scanCode({
					...
					success: function(res) {
						...
						try {
							...
							var url =
								`http://inv-veri.com/check?fpdm=${fpdm}&fphm=${fphm}&date=${date}&code=${code}&channel=${channel}`
							console.log(url)
										...
										if (data.key1 === '002') {
											...
											keys.forEach(el => {
												if (el.indexOf('fphm_') > -1) {
													_this.formData.invoiceNumber = data[
														el];
												}
												if (el.indexOf('je_') > -1) {
												...
												if (el.indexOf('jshjxx_') > -1) {
													_this.formData.costTaxesTotal = data[
														el];
												}
												if (el.indexOf('kprq_') > -1) {
													var dateFormat = /^(\d{4})[-年]?(\d{1,2})[-月]?(\d{1,2})/;
													if(dateFormat.test(data[el])){
														_this.formData.expenseDate = _this.$commonUtils.formatTimeNewBillZh(data[el]);
													}
												}
											})
											uni.hideLoading();
		...
		methods: {
			...
			ruleMoney(value, name) {
				let that = this;
				//这里计时器必须使用，不然input框中不会显示校验过的值，只显示你输入的值
				if (this.$commonUtils.isNumber(value)) {
