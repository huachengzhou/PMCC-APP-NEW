...
<script>
	...
	export default {
		...
		onNavigationBarButtonTap() {
			...
			uni.navigateTo({
				url: "./editDeclareBuildingPermit?json=" + JSON.stringify(item)
			});
		},
		computed: {
			bisDetail: function() {
		...
		methods: {
			queryPageList(pageNo, pageSize) {
				...
				let option = {
					url: "/app.assess/declare/declareBuildingPermit/getPageResult",
					query: data,
			...
			deleteItem(item) {
				...
				let option = {
					url: "/app.assess/declare/declareBuildingPermit/removeById?" + utils.parseParam(query),
					query: query,
					notParam: true,
