...
<script>
	...
	export default {
		...
		onNavigationBarButtonTap() {
			...
			uni.navigateTo({
				url: "./editDeclareLandUsePermit?json=" + JSON.stringify(item)
			});
		},
		computed: {
			bisDetail: function() {
		...
		methods: {
			queryPageList(pageNo, pageSize) {
				...
				let option = {
					url: "/app.assess/declare/declareLandUsePermit/getPageResult",
					query: data,
			...
			deleteItem(item) {
				...
				let option = {
					url: "/app.assess/declare/declareLandUsePermit/removeById?" + utils.parseParam(query),
					query: query,
					notParam: true,
