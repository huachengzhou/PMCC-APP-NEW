	...
	<view style="margin-top: 40px;">
		...
		<uni-forms :modelValue="formData" ref="form" validate-trigger="bind" err-show-type="undertext" :rules="rules">
			<uni-group title="基本信息" mode="card">
				...
				<uni-forms-item name="unit" label="用地单位" label-align="right">
					<uni-easyinput type="text" v-model="formData.unit" placeholder="用地单位" />
				</uni-forms-item>
				<uni-forms-item name="name" label="用地项目名称" label-align="right">
					<uni-easyinput type="text" v-model="formData.name" placeholder="用地项目名称" />
				</uni-forms-item>
				<uni-forms-item name="location" label="用地位置" label-align="right">
					<uni-easyinput type="text" v-model="formData.location" placeholder="用地位置" />
				</uni-forms-item>
				<uni-forms-item name="nature" label="用地性质" label-align="right">
					<uni-easyinput type="text" v-model="formData.nature" placeholder="用地性质" />
				</uni-forms-item>
				<uni-forms-item name="area" label="用地面积" label-align="right">
					<uni-easyinput type="digit" v-model="formData.area" placeholder="用地面积" />
				</uni-forms-item>
	...
	export default {
		...
		methods: {
			submit(data) {
				var _this = this;
				let option = {
					url: "/app.assess/declare/declareLandUsePermit/saveAndUpdate",
					showLoading: true,
					query: data,
			...
			setServerData: function() {
				...
				let option = {
					query: query,
					notParam: true,
					url: "/app.assess/declare/declareLandUsePermit/getById?" + utils.parseParam(query),
					successCallback: function(resultData) {
						if (!resultData) {
							return false;
						}
