	...
	export default {
		data() {
			return {
				...
				query: {},
				projectPlanDetails: {},
				fieldList: declareCommon.toDeclareRealtyCheckListDieldList(),
				DetailList: [],
				searchValue: null,
				pulldownLoad: false,
				top: '30px',
		...
		onNavigationBarButtonTap() {
			...
			let item = {
				marsterId: data.centerId,
				planDetailsId: projectPlanDetails.id
			};
			uni.navigateTo({
				url: "./editDeclareRealtyCheckList?json=" + JSON.stringify(item)
			});
		},
		computed: {
			bisDetail: function() {
		...
		methods: {
			queryPageList(pageNo, pageSize) {
				...
				var data = {
					pageNum: pageNo,
					pageSize: pageSize,
					marsterId: batchDetail.centerId
				};
				if (!data.marsterId) {
					return false;
				}
				let option = {
					url: "/app.assess/declare/declareRealtyCheckList/getPageResult",
					query: data,
			...
			deleteItem(item) {
				...
				let option = {
					url: "/app.assess/declare/declareRealtyCheckList/removeById?" + utils.parseParam(query),
					query: query,
					notParam: true,
