	...
	<view>
		<view>
			...
			<view v-show="bisNotCert">
				<data-cnpm-model @submit="submit" ref="declareRealtyLandModel" :dataId="data.id" :bisCenter="bisCenter"
					:controlParam="controlParam" :bisDetail="bisDetail">
					<uni-group title="附件" mode="card" slot="foot">
						<uni-forms-item name="declareRealtyLandCertFileId" label="土地证附件" label-align="right">
							<common-image-picker ref="certFileCpn" :dataObj="setImgFieldsName('declareRealtyLandCertFileId')">
							</common-image-picker>
						</uni-forms-item>
					</uni-group>
				</data-cnpm-model>
			...
			<view v-show="!bisNotCert">
				<data-cnpm-NotCert-model @submit="submit" ref="declareRealtyLandNotCertModel" :dataId="data.id" :bisCenter="bisCenter"
					:controlParam="controlParam" :bisDetail="bisDetail">
					<uni-group title="附件" mode="card" slot="foot">
						<uni-forms-item name="declareRealtyLandCertFileId" label="批文附件" label-align="right">
							<common-image-picker ref="certFileCpn2" :dataObj="setImgFieldsName('declareRealtyLandCertFileId')">
							</common-image-picker>
						</uni-forms-item>
					</uni-group>
...
<script>
	...
	import declareRealtyLandNotCertModel from '@/components/assess/declare/declareRealtyLandNotCertModel.vue';
	...
	export default {
		...
		components: {
			...
			"data-cnpm-NotCert-model": declareRealtyLandNotCertModel,
		},
		computed: {
			...
			assessCommonUtils.loadAsyncDataDicByKey(AssessDicKey.projectDeclareCertificateIsnull,
				(resultData => {
					if (resultData) {
						...
						this.$nextTick(() => {
							//模态框 设置数据,并且初始化
							this.$refs.declareRealtyLandModel.setServerData();
							this.$refs.declareRealtyLandNotCertModel.setServerData();
