		...
		<uni-forms :modelValue="formData" ref="form" validate-trigger="bind" err-show-type="undertext" :rules="rules">
			...
			<uni-group title="基本信息" mode="card">
				<uni-forms-item required name="name" label="名称" label-align="right">
					<select-input :dataValue="formData.name" :dataInput="'name'" :selectDatas="nameList"
				...
				<uni-forms-item name="area" label="使用面积(m²)" label-align="right">
					<uni-easyinput type="text" v-model="formData.area" placeholder="使用面积(m²)" />
				</uni-forms-item>

				<uni-forms-item name="layerHeight" label="层高(m)" label-align="right">
				...
				<uni-forms-item name="clearHeight" label="净高(m)" label-align="right">
					<select-input :dataValue="formData.clearHeight" :dataInput="'clearHeight'"
						:selectDatas="clearHeightList" @dataResult="initFormInputSelectResult">
					</select-input>
				</uni-forms-item>

				<uni-forms-item v-show="currentFloorFlag" name="currentFloor" label="所在楼层" label-align="right">
					<uni-easyinput type="text" v-model="formData.currentFloor" placeholder="所在楼层" />
				</uni-forms-item>
				...
				<uni-forms-item name="house_room_image" label="房间图片" label-align="right">
					<common-image-picker :dataObj="setImgFieldsName('house_room_image')">
					</common-image-picker>
				</uni-forms-item>
			</uni-group>
	...
	export default {
		data() {
			return {
				...
				clearHeightList: [2.4, 2.8, 3, 3.2, 2.6, 4.2, 4.8, 6, 7.2],
			...
			initListData() {
				...
				(function(tableId) {
					...
					let option = {
						query: query,
						notParam: true,
						url: "/app.assess/basicHouseHuxing/getHuxingByHouseId?" + utils.parseParam(query),
						successCallback: function(resultData) {
							if (!resultData) {
								return false;
							}
							_this.huxingObj = resultData;
							if (resultData.spatialDistribution) {
								estateCommonUtils.getDataDicInfo(resultData.spatialDistribution, function(
									dicData) {
									if (dicData) {
										if (dicData.name == '多层') {
											_this.currentFloorFlag = true;
										}
									}
								});
							}
