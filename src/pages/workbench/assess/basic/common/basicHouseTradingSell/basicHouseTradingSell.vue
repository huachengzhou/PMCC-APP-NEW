			...
			<z-paging ref="paging" :fixed=true @query="queryPageList" v-model="DetailList">
				...
				<!-- <view class="uni-container">
					<uni-table :loading="loading" border stripe emptyText="暂无更多数据">
						<uni-tr>
							<uni-th align="center">分期支付时间起</uni-th>
							<uni-th align="center">分期支付时间止</uni-th>
							<uni-th align="center">分期支付利息</uni-th>
							<uni-th align="center">操作</uni-th>
						</uni-tr>
						<uni-tr v-for="(itemData, index) in DetailList" :key="index">
							<uni-td>{{formatTime(itemData.instalmentPeriodStartName)}} </uni-td>
							<uni-td>{{formatTime(itemData.instalmentPeriodEndName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.instalmentInterest)}} </uni-td>
							<uni-td>
...
<script>
	...
	let fieldList = [{
			label: "分期支付利息",
			field: "instalmentInterest"
		},
		{
			label: "分期支付时间起",
			field: "instalmentPeriodStartName"
		},
		{
			label: "分期支付时间止",
			field: "instalmentPeriodEndName"
		},
	...
	export default {
		...
		onNavigationBarButtonTap() {
			...
			uni.navigateTo({
				url: "./basicHouseTradingSellEdit?" + utils.parseParam(param)
			});
		},
		onLoad(option) {
			let cardObj = {};
