			...
			<z-paging ref="paging" :fixed=true @query="queryPageList" v-model="DetailList">
				...
				<!-- <view class="uni-container">
					<uni-table :loading="loading" border stripe emptyText="暂无更多数据">
						<uni-tr>
							<uni-th align="center">机构名称</uni-th>
							<uni-th align="center">机构等级</uni-th>
							<uni-th align="center">机构床位数</uni-th>
							<uni-th align="center">机构距离</uni-th>
							<uni-th align="center">操作</uni-th>
						</uni-tr>
						<uni-tr v-for="(itemData, index) in DetailList" :key="index">
							<uni-td>{{getItemShowName(itemData.organizationName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.organizationLevelName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.bedNumberName)}} </uni-td>
				...
				<view class="box-bottom" slot="bottom">
					<view>
						<common-map-poi-search :position="position" :search="search" :querytype="querytype"
							@outputResult="mapCallback" :dataFilterDicByKey="dataFilterDicByKey">
...
<script>
	...
	export default {
		data() {
			return {
				dataFilterDicByKey: AssessDicKey.DATA_BASE_MAP_FILTER_MEDICAL,
				fieldList: fieldList,
				cardObj: null,
				batchDetail: null,
				DetailList: [],
		...
		onNavigationBarButtonTap() {
			...
			uni.navigateTo({
				url: "./basicMatchingMedicalEdit?" + utils.parseParam(param)
			});
		},
		onBackPress() {
		...
		methods: {
			...
			mapCallback(result) {
				let _this = this;
				var userInfo = _this.$helper.fetchLocalUser();
				let url = "/app.assess/basicMatchingMedical/addMapData";
				let query = {
