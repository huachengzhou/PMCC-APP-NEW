				...
				<view class="box-bottom" slot="bottom">
					<view>
						<common-map-poi-search :funType="1" ref="mapPoi" :position="position" :search="search"
							:querytype="querytype" @outputResult="mapCallback" :dataKey="dataKey"
							:dataFilterDicByKey="dataFilterDicByKey">
...
<script>
	...
	export default {
		data() {
			return {
				dataFilterDicByKey: AssessDicKey.DATA_BASE_MAP_FILTER_TRAFFICHUB,
				fieldList: fieldList,
				cardObj: null,
				batchDetail: null,
				DetailList: [],
		...
		onNavigationBarButtonTap() {
			...
			uni.navigateTo({
				url: "./basicMatchingTrafficTrafficHubEdit?" + utils.parseParam(param)
			});
		},
		onBackPress() {
		...
		computed: {
			dataKey() {
				let key = AssessDicKey.estate_traffic_nature;
				return key;
			}
		},
		components: {
