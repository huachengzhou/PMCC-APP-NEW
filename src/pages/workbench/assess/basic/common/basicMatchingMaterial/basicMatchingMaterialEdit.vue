	...
	<view style="margin-top: 40px;">
		<uni-forms :modelValue="formData" ref="form" validate-trigger="bind" err-show-type="undertext" :rules="rules">
			<uni-group title="基本信息" mode="card">
				...
				<uni-forms-item required name="scale" label="规模" label-align="right">
					<assess-picker :dataInput="'scale'" :dataList="scaleList" :dataValue="formData.scale"
						@dataResult="initFormPickerResult">
					</assess-picker>
				</uni-forms-item>
	...
	export default {
		...
		methods: {
			...
			initListData() {
				...
				estateCommonUtils.loadAsyncDataDicByKey(AssessDicKey.estate_supply_new_distance, function(
				    resultData) {
				    if (!resultData) {
				        return false;
				    }
				...
				estateCommonUtils.loadAsyncDataDicByKey(AssessDicKey.estate_supply_new_scale, function(
				    resultData) {
				    if (!resultData) {
				        return false;
				    }
				    data.scaleList = resultData;
				});
				
				estateCommonUtils.loadAsyncDataDicByKey(AssessDicKey.estate_supply_new_type, function(
				    resultData) {
				    if (!resultData) {
				        return false;
				    }
