			...
			<z-paging ref="paging" :fixed=true @query="queryPageList" v-model="DetailList">
				...
				<!-- <view class="uni-container">
					<uni-table :loading="loading" border stripe emptyText="暂无更多数据">
						<uni-tr>
							<uni-th align="center">排水系统</uni-th>
							<uni-th align="center">类别</uni-th>
							<uni-th align="center">排水处理方式</uni-th>
							<uni-th align="center">操作</uni-th>
						</uni-tr>
						<uni-tr v-for="(itemData, index) in DetailList" :key="index">
							<uni-td>{{getItemShowName(itemData.drainSystemName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.typeName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.processingModeName)}} </uni-td>
							<uni-td>
...
<script>
	...
	export default {
		...
		onNavigationBarButtonTap() {
			...
			uni.navigateTo({
				url: "./basicHouseWaterDrainEdit?" + utils.parseParam(param)
			});
		},
		onBackPress() {
