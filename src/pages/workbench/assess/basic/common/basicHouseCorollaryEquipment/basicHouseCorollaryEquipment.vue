				...
				<!-- <view class="uni-container">
					<uni-table :loading="loading" border stripe emptyText="暂无更多数据">
						<uni-tr>
							...
							<uni-th align="center">维护状况</uni-th>
							<uni-th align="center">参数指标</uni-th>
						...
						<uni-tr v-for="(itemData, index) in DetailList" :key="index">
							...
							<uni-td>{{getItemShowName(itemData.equipmentUseName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.maintenanceStatusName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.parameterIndex)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.price)}} </uni-td>
...
<script>
	...
	export default {
		...
		onNavigationBarButtonTap() {
			...
			uni.navigateTo({
				url: "./basicHouseCorollaryEquipmentEdit?" + utils.parseParam(param)
			});
		},
		onBackPress() {
