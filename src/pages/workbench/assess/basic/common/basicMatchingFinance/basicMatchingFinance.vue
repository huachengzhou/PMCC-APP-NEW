			...
			<z-paging ref="paging" :fixed=true @query="queryPageList" v-model="DetailList">
				...
				<!-- <view class="uni-container">
					<uni-table :loading="loading" border stripe emptyText="暂无更多数据">
						<uni-tr>
							<uni-th align="center">金融名称</uni-th>
							<uni-th align="center">与金融机构的距离</uni-th>
							<uni-th align="center">金融类别</uni-th>
							<uni-th align="center">金融机构性质</uni-th>
							<uni-th align="center">服务类别</uni-th>
...
<script>
	...
	export default {
		data() {
			return {
				dataFilterDicByKey: AssessDicKey.DATA_BASE_MAP_FILTER_FINANCE,
				fieldList: fieldList,
				cardObj: null,
				batchDetail: null,
				DetailList: [],
		...
		onNavigationBarButtonTap() {
			...
			uni.navigateTo({
				url: "./basicMatchingFinanceEdit?" + utils.parseParam(param)
			});
		},
		onLoad(option) {
			let cardObj = {};
		...
		computed: {
			dataKey() {
				let key = AssessDicKey.estate_finance_category;
				return key;
			}
		},
		methods: {
			...
			mapCallback(result, ) {
				let _this = this;
				var userInfo = _this.$helper.fetchLocalUser();
				let url = "/app.assess/basicMatchingFinance/addMapData";
				let query = {
