	...
	<view style="margin-top: 40px;">
		<uni-forms :modelValue="formData" ref="form" validate-trigger="bind" err-show-type="undertext" :rules="rules">
			<uni-group title="基本信息" mode="card">
				...
				<uni-forms-item required name="materialQuality" label="材质" label-align="right">
					<assess-picker :dataInput="'materialQuality'" :dataList="materialQualityList"
						:dataValue="formData.materialQuality" @dataResult="initFormPickerResult">
					</assess-picker>
				</uni-forms-item>


				<uni-forms-item required name="category" label="分类" label-align="right">
	...
	export default {
		...
		methods: {
				...
				switch (name) {
					case "type" :{
						estateCommonUtils.getDataDicListByPid(formData[name], function(resultData) {
							...
							data.materialQualityList = resultData;
						});
						break ;
					}
					default: {
			...
			initListData() {
				...
				estateCommonUtils.loadAsyncDataDicByKey(AssessDicKey.examine_building_maintenance_category, function(
				    resultData) {
				    if (!resultData) {
				        return false;
				    }
				...
				estateCommonUtils.loadAsyncDataDicByKey(AssessDicKey.examine_building_maintenance_type, function(
				    resultData) {
				    if (!resultData) {
				        return false;
				    }
