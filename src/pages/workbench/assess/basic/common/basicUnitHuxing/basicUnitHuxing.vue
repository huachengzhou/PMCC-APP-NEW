	...
	<view class="content">
		...
		<view>
			<z-paging ref="paging" :fixed=true @query="queryPageList" v-model="DetailList">
				...
				<view>
					<common-custom-panel-grid :icon="false" :titles="fieldList" :list="DetailList"  @editItem="editItem"
				...
				<!-- <view class="uni-container">
					<uni-table :loading="loading" border stripe emptyText="暂无更多数据">
						<uni-tr>
							<uni-th align="center">户型</uni-th>
							<uni-th align="center">面积</uni-th>
							<uni-th align="center">朝向</uni-th>
							<uni-th align="center">空间布局</uni-th>
						...
						<uni-tr v-for="(itemData, index) in DetailList" :key="index">
							...
							<uni-td>{{getItemShowName(itemData.area)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.orientationName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.spatialDistributionName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.description)}} </uni-td>
							<uni-td>
...
<script>
	...
	export default {
		...
		onNavigationBarButtonTap() {
			...
			// };
			// uni.navigateTo({
			// 	url: "./basicUnitHuxingEdit?" + utils.parseParam(param)
			// });
			uni.showToast({
				title: '单元户型信息不能添加',
			});
