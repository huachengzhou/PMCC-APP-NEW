				...
				<!-- <view class="uni-container">
					<uni-table :loading="loading" border stripe emptyText="暂无更多数据">
						<uni-tr>
							...
							<uni-th align="center">学校性质</uni-th>
							<uni-th align="center">学校级次</uni-th>
							<uni-th align="center">学校等级</uni-th>
							<uni-th align="center">距离</uni-th>
							<uni-th align="center">操作</uni-th>
						</uni-tr>
						<uni-tr v-for="(itemData, index) in DetailList" :key="index">
							<uni-td>{{getItemShowName(itemData.schoolName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.schoolNatureName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.schoolGradationName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.schoolLevelName)}} </uni-td>
				...
				<view class="box-bottom" slot="bottom">
					<view>
						<common-map-poi-search :funType="1" ref="mapPoi" :position="position" :search="search"
							:querytype="querytype" @outputResult="mapCallback" :dataFilterDicByKey="dataFilterDicByKey"
							:dataKey="dataKey">
...
<script>
	...
	let fieldList = [{
		...
		{
			label: "学校性质",
			field: "schoolNatureName"
		},
		{
			label: "学校级次",
			field: "schoolGradationName"
		},
		{
			label: "学校等级",
	...
	export default {
		data() {
			return {
				dataFilterDicByKey: AssessDicKey.DATA_BASE_MAP_FILTER_EDUCATION,
				fieldList: fieldList,
				cardObj: null,
				batchDetail: null,
				DetailList: [],
		...
		onNavigationBarButtonTap() {
			...
			uni.navigateTo({
				url: "./basicMatchingEducationEdit?" + utils.parseParam(param)
			});
		},
		onBackPress() {
		...
		computed: {
			dataKey() {
				let key = AssessDicKey.estate_school_gradation;
				return key;
			}
		},
		components: {
		...
		methods: {
			...
			mapCallback(result, ) {
				let _this = this;
				var userInfo = _this.$helper.fetchLocalUser();
				let url = "/app.assess/basicMatchingEducation/addMapData";
				let query = {
