			...
			<z-paging ref="paging" :fixed=true @query="queryPageList" v-model="DetailList">
				...
				<!-- <view class="uni-container">
					<uni-table :loading="loading" border stripe emptyText="暂无更多数据">
						<uni-tr>
							<uni-th align="center">停车场名称</uni-th>
							<uni-th align="center">车位位置</uni-th>
							<uni-th align="center">车位类型</uni-th>
							<uni-th align="center">停车场类别</uni-th>
							<uni-th align="center">车位数量</uni-th>
							<uni-th align="center">车位图片</uni-th>
							<uni-th align="center">操作</uni-th>
						</uni-tr>
						<uni-tr v-for="(itemData, index) in DetailList" :key="index">
							<uni-td>{{getItemShowName(itemData.name)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.locationName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.parkingTypeName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.parkingEstateName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.number)}} </uni-td>
							<uni-td>
								<common-image-picker :dataObj="setImgFieldsName(itemData)">
								</common-image-picker>
							</uni-td>
							<uni-td>
...
<script>
	...
	let fieldList = [{
		...
		{
			label: "车位类型",
			field: "parkingTypeName"
		},
		{
			label: "停车场类别",
			field: "parkingEstateName"
		},
		{
			label: "车位数量",
			field: "number"
	...
	export default {
		...
		onNavigationBarButtonTap() {
			...
			uni.navigateTo({
				url: "./basicEstateParkingEdit?" + utils.parseParam(param)
			});
		},
		onBackPress() {
			let batchDetail = this.batchDetail;
