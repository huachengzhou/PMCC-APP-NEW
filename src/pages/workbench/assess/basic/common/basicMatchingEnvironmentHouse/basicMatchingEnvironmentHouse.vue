<template>
	<view class="content">
			...
			<z-paging ref="paging" :fixed=true @query="queryPageList" v-model="DetailList">
				...
				<!-- <view class="uni-container">
					<uni-table :loading="loading" border stripe emptyText="暂无更多数据">
						<uni-tr>
							<uni-th align="center">景观类型</uni-th>
							<uni-th align="center">影响要素</uni-th>
							<uni-th align="center">景观描述</uni-th>
							<uni-th align="center">影响结论</uni-th>
						...
						<uni-tr v-for="(itemData, index) in DetailList" :key="index">
							...
							<uni-td>{{getItemShowName(itemData.humanImpactName)}} </uni-td>
							<uni-td>
		...
		<environment-quote @getResults="getResults" :dataValue="estateId" ref="houseQuote"></environment-quote>
...
<script>
	...
	import assessBasicHouseQuote from '@/components/assess/common/assess-basic-environment-house-quote/assess-basic-environment-house-quote.vue';
	...
	export default {
		...
		onNavigationBarButtonTap(e) {
			...
			if (index == 0) {
				uni.navigateTo({
					url: "./basicMatchingEnvironmentHouseEdit?" + utils.parseParam(param)
				});
			} else if (index == 1) {
		...
		components: {
			"environment-quote": assessBasicHouseQuote,
		...
		methods: {
			...
			getResults(rows) {
				...
				let option = {
					notParam: true,
					query: query,
					url: "/app.assess/basicHouseEnvironment/itemReferenceDataByIds?" + utils.parseParam(query),
					successCallback: function() {
