				...
				<!-- <view class="uni-container">
					<uni-table :loading="loading" border stripe emptyText="暂无更多数据">
						<uni-tr>
							...
							<uni-th align="center">所在线路</uni-th>
							<uni-th align="center">收费标准</uni-th>
						...
						<uni-tr v-for="(itemData, index) in DetailList" :key="index">
							...
							<uni-td>{{getItemShowName(itemData.theLine)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.costStandard)}} </uni-td>
							<uni-td>
...
<script>
	...
	export default {
		data() {
			return {
				dataFilterDicByKey: AssessDicKey.DATA_BASE_MAP_FILTER_TRANSIT,
				fieldList: fieldList,
				cardObj: null,
				batchDetail: null,
				DetailList: [],
		...
		onNavigationBarButtonTap() {
			...
			uni.navigateTo({
				url: "./basicMatchingTrafficTransitEdit?" + utils.parseParam(param)
			});
		},
		onBackPress() {
		...
		methods: {
			...
			mapCallback(result) {
				let _this = this;
				var userInfo = _this.$helper.fetchLocalUser();
				let url = "/app.assess/basicMatchingTraffic/addMapData";
				let query = {
