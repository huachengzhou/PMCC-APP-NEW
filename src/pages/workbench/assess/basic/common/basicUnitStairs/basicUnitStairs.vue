			...
			<z-paging ref="paging" :fixed=true @query="queryPageList" v-model="DetailList">
				...
				<!-- <view class="uni-container">
					<uni-table :loading="loading" border stripe emptyText="暂无更多数据">
						<uni-tr>
							<uni-th align="center">楼梯类型</uni-th>
							<uni-th align="center">用途</uni-th>
							<uni-th align="center">楼梯间</uni-th>
							<uni-th align="center">操作</uni-th>
						</uni-tr>
						<uni-tr v-for="(itemData, index) in DetailList" :key="index">
							<uni-td>{{getItemShowName(itemData.type)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.purpose)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.staircase)}} </uni-td>
							<uni-td>
...
<script>
	...
	export default {
		...
		onNavigationBarButtonTap() {
			...
			uni.navigateTo({
				url: "./basicUnitStairsEdit?" + utils.parseParam(param)
			});
		},
		onBackPress() {
