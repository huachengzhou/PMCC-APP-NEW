				...
				<!-- <view class="uni-container">
					<uni-table :loading="loading" border stripe emptyText="暂无更多数据">
						<uni-tr>
							...
							<uni-th align="center">服务时间</uni-th>
							<uni-th align="center">等级评价</uni-th>
							<uni-th align="center">操作</uni-th>
						</uni-tr>
						<uni-tr v-for="(itemData, index) in DetailList" :key="index">
							<uni-td>{{getItemShowName(itemData.serviceTypeName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.serviceContentName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.serviceTime)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.gradeEvaluationName)}} </uni-td>
							<uni-td>
...
<script>
	...
	export default {
		...
		onNavigationBarButtonTap() {
			...
			uni.navigateTo({
				url: "./basicBuildingPropertyServiceItemEdit?" + utils.parseParam(param)
			});
		},
		onLoad(option) {
			let cardObj = {};
