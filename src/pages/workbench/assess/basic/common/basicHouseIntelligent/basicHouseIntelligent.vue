			...
			<z-paging ref="paging" :fixed=true @query="queryPageList" v-model="DetailList">
				...
				<!-- <view class="uni-container">
					<uni-table :loading="loading" border stripe emptyText="暂无更多数据">
						<uni-tr>
							<uni-th align="center">开关回路</uni-th>
							<uni-th align="center">档次</uni-th>
							<uni-th align="center">铺设方式</uni-th>
							<uni-th align="center">灯具</uni-th>
							<uni-th align="center">智能系统</uni-th>
							<uni-th align="center">智能系统描述</uni-th>
							<uni-th align="center">操作</uni-th>
						</uni-tr>
						<uni-tr v-for="(itemData, index) in DetailList" :key="index">
							<uni-td>{{getItemShowName(itemData.switchCircuitName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.gradeName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.layingMethodName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.lampsLanternsName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.intelligentSystemName)}} </uni-td>
							<uni-td>{{getItemShowName(itemData.systemDescribe)}} </uni-td>
							<uni-td>
...
<script>
	...
	let fieldList = [{
		...
		{
			label: "铺设方式",
			field: "layingMethodName"
		},
		{
			label: "灯具",
			field: "lampsLanternsName"
		},
		{
			label: "智能系统",
			field: "intelligentSystemName"
		},
		{
			label: "智能系统描述",
	...
	export default {
		...
		onNavigationBarButtonTap() {
			...
			uni.navigateTo({
				url: "./basicHouseIntelligentEdit?" + utils.parseParam(param)
			});
		},
		onBackPress() {
