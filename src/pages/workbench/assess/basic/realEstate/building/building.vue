	...
	<view style="margin-top: 40px;">
		...
		<uni-forms :modelValue="formData" ref="form" validate-trigger="bind" err-show-type="undertext" :rules="rules">
			<uni-group title="基本信息" mode="card">
				...
				<uni-forms-item required name="maxFloor" label="最高层" label-align="right">
					<uni-easyinput type="text" v-model="formData.maxFloor" placeholder="最高层" />
				</uni-forms-item>
				<uni-forms-item required name="floorCount" label="总层数" label-align="right">
				...
				<uni-forms-item required name="appearanceStyle" label="外观风格" label-align="right">
				...
				<uni-forms-item required name="facilitiesUseFee" label="配套公共设施使用费" label-align="right">
					<uni-easyinput type="text" v-model="formData.facilitiesUseFee" placeholder="配套公共设施使用费" />
				</uni-forms-item>


				<uni-forms-item required name="appearanceNewAndOld" label="外观新旧" label-align="right">
				...
				<uni-forms-item required name="betweenDistance" label="楼间距" label-align="right">
				...
				<uni-forms-item required name="minimumFloorDistance" label="最小楼间距倍数" label-align="right">
					<select-input :dataValue="formData.minimumFloorDistance" :dataInput="'minimumFloorDistance'"
						:selectDatas="minimumFloorDistanceList" @dataResult="initFormInputSelectResult">
					</select-input>
				</uni-forms-item>


				<uni-forms-item required name="completedTimeType" label="竣工资料获取方式" label-align="right">
				...
				<uni-forms-item v-show="completedTimeTypeName != '无法获取' " required name="beCompletedTime" label="竣工时间" label-align="right">
				...
				<uni-forms-item v-show="completedTimeTypeName != '无法获取' " required name="constructionQuality" label="工程质量" label-align="right">
				...
				<uni-forms-item required name="openTime" label="开盘时间" label-align="right">
					<uni-datetime-picker type="date" :value="formData.openTime"
						v-on:change="setFormValue('openTime',$event)"></uni-datetime-picker>
				</uni-forms-item>
				
				<uni-forms-item required name="propertyFee" label="物业费(平方米)" label-align="right">
			...
			<uni-group title="补充信息" mode="card">
				<uni-forms-item name="buildingArea" label="建筑面积(平方米)" label-align="right">
					<uni-easyinput type="text" v-model="formData.buildingArea" placeholder="建筑面积(平方米)" />
				</uni-forms-item>
				...
				<uni-forms-item name="propertySocialPrestige" label="物业公司社会信誉" label-align="right">
				...
				<uni-forms-item name="propertyCompanyNature" label="物业公司性质" label-align="right">
				...
				<uni-forms-item name="propertyName" label="物业公司名称" label-align="right">
		...
		<assess-basic-house-foot-nav ref="houseFootNav" :dataId="batchDetail.pid"
...
<script>
	...
	let thisViewData = {
		...
		minimumFloorDistanceList: ['0.5', '1', '1.5', '2'],
		appearanceStyleList: [],
		streetInfoIdList: [],
	...
	export default {
		...
		computed: {
			completedTimeTypeName: function() {
				let text = "";
				let completedTimeTypeList = this.completedTimeTypeList;
				let completedTimeType = this.formData.completedTimeType;
				if (utils.isArrayNotEmpty(completedTimeTypeList) && utils.isNotEmptyString(completedTimeType)) {
					if (utils.isNumber(completedTimeType)) {
						completedTimeType = Number(completedTimeType);
						completedTimeTypeList.forEach(function(item) {
							if (item.id == completedTimeType) {
								text = item.name;
							}
						});
					}
		...
		onLoad(option) {
			...
			//提交按钮控制
			estateCommonUtils.runBasicCheckWriteContent(this.homeParam.applyBatchId,this.$helper.fetchLocalUser(),this.$data) ;
