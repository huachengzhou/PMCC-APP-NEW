<template>
    ...
    <view style="margin-top: 40px;">
        ...
        <uni-forms :modelValue="houseData" ref="formHouse" validate-trigger="bind" err-show-type="undertext"
                   :rules="houseRules">

            <uni-group mode="card">
                <uni-forms-item required name="area" label="面积" label-align="right">
                    <uni-easyinput type="text" v-model="houseData.area" placeholder="面积"/>
                </uni-forms-item>
            </uni-group>
        ...
        <uni-forms :modelValue="formData" ref="form" validate-trigger="bind" err-show-type="undertext" :rules="rules">
            <uni-group title="基本信息" mode="card">
                ...
                <uni-forms-item name="transactionLevel" label="市场级别" label-align="right">
                    <assess-picker :dataInput="'transactionLevel'" :dataList="transactionLevelList"
                                   :dataValue="formData.transactionLevel" @dataResult="initFormPickerResult">
                    </assess-picker>
                </uni-forms-item>
                ...
                <uni-forms-item name="bidder" label="竞买人" label-align="right">
                    <uni-easyinput type="text" v-model="formData.bidder" placeholder="竞买人"/>
                </uni-forms-item>

            </uni-group>

            <uni-group title="支付条件" mode="card">
                ...
                <uni-forms-item name="cost" label="土地附加成本(万元)" label-align="right">
                    <uni-easyinput type="digit"
                                   v-model="formData.cost" placeholder="土地附加成本(万元)"/>
                </uni-forms-item>


                <uni-forms-item name="tradingUnitPrice" label="交易单价（元）" label-align="right">
                    <uni-easyinput v-on:blur="changeLoadData('tradingUnitPrice')" type="digit"
                                   v-model="formData.tradingUnitPrice" placeholder="交易单价（元）"/>
                </uni-forms-item>


                <uni-forms-item name="perMuPrice" label="每亩单价（元）" label-align="right">
                    <uni-easyinput type="digit"
                                   v-model="formData.perMuPrice" placeholder="每亩单价（元）"/>
                </uni-forms-item>

                <uni-forms-item name="tradingTotalPrice" label="交易总价（元）" label-align="right">
                    <uni-easyinput v-on:blur="changeLoadData('tradingTotalPrice')" type="digit"
                ...
                <uni-forms-item name="restrictions" label="限制事项" label-align="right">
                    <uni-easyinput type="text" v-model="formData.restrictions" placeholder="限制事项"/>
                </uni-forms-item>
                <uni-forms-item name="restrictionsRemark" label="限制事项说明" label-align="right">
                    <uni-easyinput type="textarea" autoHeight v-model="formData.restrictionsRemark"
                                   placeholder="限制事项说明"/>
                </uni-forms-item>
        ...
        <uni-group title="其它信息" mode="card">
            <view v-if="tradingTypeName == '出租'">
                <button type="primary" size="mini" @click="openTradingTypePage(1)">出租信息
                </button>
            </view>
            <view v-if="tradingTypeName == '出售'">
                <button type="primary" size="mini" @click="openTradingTypePage(2)">分期信息
                </button>
            </view>
        </uni-group>
        ...
        methods: {
            submit(data, commitFlag) {
                ...
                new Promise((resolved, rejected) => {
                    ...
                    let infoData = {
                        basicHouse: _this.houseData,
                        landCategoryInfo: data
                    };
                    let coreData = {
            ...
            submitForm(form) {
                ...
                var _this = this;
                let batchDetail = this.batchDetail;
                _this.$refs.formHouse.validate().then((resHouse) => {
            ...
            initListData: function () {
                ...
                let data = this.$data;
                var _this = this;
                this.changeLoadData("tradingType");
                ...
                estateCommonUtils.loadAsyncDataDicByKey(AssessDicKey.property_land_scope, function (
                    resultData) {
                    if (!resultData) {
                        return false;
                    }
                ...
                utils.requestDefault({
                    ...
                    notParam: true,
                    url: "/app.assess/data/assessDataValueDefinition/getValueDefinition?applyBatchId=" +
                        batchDetail.applyBatchId,
                ...
                switch (name) {
                    ...
                    case "tradingType": {
                        ...
                        estateCommonUtils.getDataDicInfo(value, function (item) {
                            _this.tradingTypeName = item.name;
            ...
            setServerData: function () {
                let url = "/app.assess/basicHouseTrading/getByHouseId";
                let _this = this;
                let batchDetail = this.batchDetail;
                let formData = this.formData;
                let query = {
            ...
            openTradingTypePage(key) {
                let tradingId = this.formData.id;
                let card = {
