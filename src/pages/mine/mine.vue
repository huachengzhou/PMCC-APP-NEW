<template>
	<view>
		...
		<view class="list">
			...
			<view class="li" hover-class="hover" @tap="openAboutMe()">
				<view class="icon">
		...
		<!-- {{hasLogin ? '退出登录' : '登录'}} -->
		<button type="primary" class="login-btn" @click="bindLogin">退出登录</button>
...
<script>
	...
	export default {
		...
		onLoad() {
			this.userInfo = this.$helper.fetchLocalUser();
			this.$nextTick(() => {
				this.initGetVersionFun();
			});
		},
		onShow() {
			this.$nextTick(() => {
		...
		methods: {
			...
			bindLogin() {
				...
				let res = this.$myRequest.pmccRequest({
					url: `/app.auth/logout`,
					data: {userAccount:userInfo.userAccount,deviceCode:deviceCode},
					method: "POST",
					showLoading:true
				})
				res.then(res => {
			...
			initGetVersionFun() {
				// #ifdef APP-PLUS
				this.versionInfo.softVersion = plus.runtime.version;
				plus.runtime.getProperty(plus.runtime.appid, (wgtinfo) => {
					this.versionInfo.thisVersion = wgtinfo.version;
					this.title = wgtinfo.name;
				});
				// #endif 
				try {
