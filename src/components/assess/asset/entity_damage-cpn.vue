			...
			<my-ming-pop ref="myMingPop" direction="below" :is_close="false" :is_mask="true" :width="100"
				...
				<view v-for="(item,index) in dataList">
					<uni-forms v-show="tabIndex == index" :modelValue="formData" validate-trigger="bind"
						...
						<uni-group mode="card">
							<uni-forms-item name="entityProjectName" label="项目" label-align="right">
								<uni-easyinput type="text" v-model="item.entityProjectName" placeholder="项目">
								</uni-easyinput>
							</uni-forms-item>

							<uni-forms-item name="entityProjectItem" label="明细" label-align="right">
								<uni-easyinput type="text" v-model="item.entityProjectItem" placeholder="明细">
								</uni-easyinput>
							</uni-forms-item>
	...
	export default {
		...
		methods: {
			...
			addData() {
				...
				let dataList = this.dataList;
				dataList.push({
					fileName: "entityProjectNamefile" + (time + Math.round(Math.random() * 10 + 10)),
					entityProjectItem: '',
					entityProjectName: ""
				});
