<template>
	<view>
		<select-input-mul :dataValue="dataValue" :dataInput="dataInput" :selectDatas="dataList"
...
<script>
	...
	import selectInputMul from '@/components/select-Input/select-Input-mul.vue';
	...
	export default {
		...
		components: {
			...
			"select-input-mul": selectInputMul
		},
		methods: {
			initFun: function() {
				...
				if (masterId) {
					this.$myRequest.pmccRequest(option).then(res => {
						...
						if (unitCommonParts.length != 0) {
							let oldArr = [] ;
							_this.uniq(unitCommonParts).forEach(label => {
								oldArr.push({text:label,value:label}) ;
							}) ;
							_this.dataList = oldArr;
						} else {
							uni.showToast({
