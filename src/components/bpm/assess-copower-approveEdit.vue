<template>
	...
	<view>
		<uni-forms :modelValue="formData" ref="form" validate-trigger="bind" err-show-type="undertext" :rules="rules">
			<uni-group title="审批意见" mode="card">
				<uni-forms-item v-if="bisSelectUser === '1' && ApproverList.length > 0" required name="ApproverUser"
					...
					<assess-picker :dataId="'userAccount'" :dataName="'userName'" :dataInput="'ApproverUser'" :dataList="ApproverList"
		...
		<uni-row class="uni-row" :width="nvueWidth">
			...
			<uni-col :push="1" :span="8">
				<view> <button @click="submit('approval',$event)" type="primary" size="mini">提交申请</button></view>
			</uni-col>
			<uni-col :push="1" :span="8">
...
<script>
	...
	export default {
		...
		methods: {
			submit(conclusion, e) {
				//普通提交
				let webUrl = "/app.assess/basic/public/taskEditSubmit";
				if (this.bisProjectFlag) {
					webUrl = "/app.assess/basic/public/submitProjectTaskEdit";
					//走项目提交
				}
				this.$refs.form.validate().then(async (res) => {
					...
					var data = {
						...
						taskRemarks: userInfo.userAccount,
						actualHours:'10',
