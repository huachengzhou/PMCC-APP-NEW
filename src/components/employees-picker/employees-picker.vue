<template>
	<view>
		<view>
			<view>
				<uni-easyinput  :clearable="false" v-model="selectedResults" @focus="focus('draw')" placeholder="请选择员工"></uni-easyinput>
			</view>
				...
				<view class="view">
					<z-paging ref="paging" fixed @query="queryPageList" v-model="dataList">
						...
						<view class="box-bottom" slot="bottom" >
							<view @click="showSelectedDrawer('draw1')" class="text"><text style="color: #007AFF;">{{multiple ? value.length>0?'已选择:'+value.length+'人':'已选择:': typeof value == 'number'?'已选择:1人':'已选择:'}}</text></view>
		...
		<view>
			<uni-drawer  ref="draw1" mode="right" mask :width="drawerWidth" @change="drawchange($event,'showdraw1')">
				<view >
					<view class="box-bottom" >
						<view class="text"><text >{{multiple ? value.length>0?'已选择数量:'+value.length+'人':'已选择数量:0' : typeof value == 'number'?'已选择数量:1':'已选择数量:0'}}</text></view>
...
<script>
	export default {
		name:"employees-picker",
		//父传值给子
		props:{
			inputValue:{type:Array,default:''},
			multiple:{type:Boolean,default:true},
			currOrg:{type:Boolean,default:false}
		},
		data() {
			return {
				//抽屉 是否打开
			...
			inputValue(data){
				...
				if(this.multiple){
					...
					if(Array.isArray(eList) && eList.length > 0){
						...
						this.selectedDatas = eList[0];
						this.selectedResults = eList[0].userName;
					}
				}
			}
		},
		...
		methods: {
			...
			theMultiple(e){
				...
				if(this.dataList.length > 0){
					if(this.value.length>0){
						this.dataList.forEach((user) => {
							this.unselecteds.push(user.value);
						});
						this.value.forEach((id) => {
							this.unselecteds.forEach((user,index) =>{
								if(user === id){
				...
				this.selectedsAll = uni.getStorageSync("selectedsAll")
				if(this.selectedsAll.length > 0){
					this.unselecteds.forEach((user) =>{
				...
				var datas = uni.getStorageSync("selectedDatas")
				if(datas.length > 0){
					this.unselecteds.forEach((user) =>{
			...
			async queryPageList(pageNo, pageSize) {
				...
				if (res.data.ret) {
					...
					if(this.multiple){
						uni.setStorageSync("selectedsAll",this.value);
						uni.setStorageSync("selectedDatas",this.selectedDatas);
					}
			...
			drawchange(e, type) {
				this[type] = e
				let drawType = {showdraw: this.showdraw,showdraw1:this.showdraw1 }
				this.$emit("getdrawType",drawType);
			},
			//关闭抽屉
			closedraw(dramName){
				this.$refs[dramName].close();
				//手动关闭软键盘
				uni.hideKeyboard();
			}
		}
